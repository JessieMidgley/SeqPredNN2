...
class Sampling:
    ...
    @staticmethod
    def oversample(indices, sample_size):
        # draw random sample of indices from each amino acid class for the training set
        sample_idx = []
        for amino_acid in indices:
            sample_idx.append(np.random.choice(indices[amino_acid], sample_size, replace=True))
    ...
    def balanced(self):
        ...
        # draw random sample of indices from each amino acid class for the validation set
        indices = list(self.idx_dict)
        validation_size = int(len(indices) * (1 - self.train_ratio))
        validation_idx = np.random.choice(indices, validation_size, replace=False)
        print('Validation set:', validation_size, 'randomly chosen residues\n')
        # all remaining residues are part of the training set
        available_idx = {float(i): [] for i in range(20)}
        for amino_acid in residue_idx:
            available_idx[amino_acid] = [idx for idx in residue_idx[amino_acid] if not np.isin(idx, validation_idx)]
        new_frequencies = {residue: len(indices) for residue, indices in available_idx.items()}
        # sample size for each amino acid class is the frequency of the most common amino acid class
        train_size = max(new_frequencies.values())
        print('Training set:', train_size, 'residues per amino acid class')
        train_idx = self.oversample(available_idx, train_size)
...
if __name__ == "__main__":
    ...
    # load model parameters from the first training phase
    model.load_state_dict(torch.load(out_dir / "unbalanced_parameters.pth"))
    ...
    torch.save(model.state_dict(), out_dir / "TPT-2-OS_parameters.pth")
    print("Saved PyTorch Model State to TPT-2-OS_parameters.pth")
